#!/usr/bin/env python3
"""
Side-view debug script with parallel, non‑crossing row/column lines.
Rows and columns each share the same slope, preventing overlaps.
"""
...
# ================================================================
# Geometry computation – parallel lines (no crossing)
# ================================================================
...
def compute_geometry(centers, boxes, is_left=True):
    ...
    for r in row_bboxes:
        if len(r) > 0:
            ...
            if ab is not None:
                ...
                row_intercepts.append(ab[1])
            else:
                # fallback: horizontal
                avg_y = np.mean([centers[idx, 1] for idx in r])
                row_slopes.append(0.0)
                row_intercepts.append(avg_y)
        else:
            row_slopes.append(0.0)
            row_intercepts.append(0.0)

    # Average slope across all rows (ignore outliers? we use median)
    if row_slopes:
        avg_row_slope = np.median(row_slopes)
    else:
        avg_row_slope = 0.0

    # Recompute intercepts using the common slope: b = y_avg - avg_slope * x_avg
    row_lines = []
    for i, r in enumerate(row_bboxes):
        if len(r) > 0:
    ...
    col_slopes = []
    col_intercepts = []
    cluster_centroids = []   # store (avg_x, avg_y) for each cluster
    ...
    for cluster in clusters:
        ...
        if ab is not None:
            ...
            col_intercepts.append(ab[1])
        else:
            avg_x = np.mean([p[0] for p in pts])
            avg_y = np.mean([p[1] for p in pts])
            col_slopes.append(0.0)
            col_intercepts.append(avg_x)
        # store average point for later
        avg_x = np.mean([p[0] for p in pts])
        avg_y = np.mean([p[1] for p in pts])
        cluster_centroids.append((avg_x, avg_y))

    if col_slopes:
        avg_col_slope = np.median(col_slopes)
    else:
        avg_col_slope = 0.0

    # Recompute column intercepts using common slope: b = x_avg - avg_slope * y_avg
    column_lines = []
    col_counts = {}
    col_center_counts = {}
    ...
    for col_idx, (cluster, (avg_x, avg_y)) in enumerate(zip(clusters, cluster_centroids)):
        ...
        # Count distinct rows in this column
        all_rows = set()
    ...
    return {
        ...
        "row_lines": row_lines,          # (a,b) for y = a*x+b, all with same a
        "column_lines": column_lines     # (a,b) for x = a*y+b, all with same a
    }


# ================================================================
# Annotation – bold boxes, parallel lines, highlighted centroids
# ================================================================
...
def draw_row_lines(img, row_lines, row_counts):
    ...
    for i, (a, b) in enumerate(row_lines):
        ...
        y1 = max(0, min(h-1, y1))
        y2 = max(0, min(h-1, y2))
        cv2.line(img, (x1, y1), (x2, y2), ROW_COLOR, LINE_THICKNESS)
        if i < len(row_counts):
            cv2.putText(img, f"R{i}: {row_counts[i]}", (10, max(20, y1 - 8)),
...
def draw_column_lines(img, column_lines, column_counts):
    ...
    for i, (a, b) in enumerate(column_lines):
        ...
        x1 = max(0, min(w-1, x1))
        x2 = max(0, min(w-1, x2))
        cv2.line(img, (x1, y1), (x2, y2), COL_COLOR, LINE_THICKNESS)
